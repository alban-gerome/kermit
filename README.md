...
If you are based in Europe like me you may only collect web analytics data from a visitor after they have provided consent. ~~Kermit provides support for tagging elements for implicit consent. This requires a separate module.~~ On Jan 9th, 2019 the Privacy and Electronic Communications Regulations (PECR) so implicit consent is no longer an option. Analytics tracking now requires explcit consent only. As of Kermit 0.8.4 implicit consent tracking using ```data-analytics-consent-implicit``` is deprecated.
